...
class ApiClient {
    constructor(baseUrl) {       // Constructor gets base url for client
        ...
        this.queryResults = document.getElementById("response");  // Get text area to output response
    }
...
const apiClient = new ApiClient('https://jsonplaceholder.typicode.com/users');   // Instanciate apiClient object to process requests
...
function postPutPatchRequest(method) {
    ...
    inputs.forEach(input => {                              // Check each input field for values
        ...
        if (input.value) {                                 // If an input field contains a value, add the value to the jsonFields object
            ...
            const currentID = +input.id.substring(1);   // Each input id has to begin with a letter.
